...
public class VncCanvas extends ImageView {
    ...
    private boolean isLGPhone = false;
    ...
    public Handler handler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            if (msg.what == 123) {
                    String orientation = (String) msg.obj;
                    if (orientation != null && orientation.length() != 0) {
                        serverScreenWidth = Integer.parseInt(orientation.split(";")[1].split(":")[1].split("\\*")[1]);
                        serverScreenHeight = Integer.parseInt(orientation.split(";")[1].split(":")[1].split("\\*")[0]);
                        int tempVar;
                        if(serverScreenWidth>serverScreenHeight){
                           tempVar=serverScreenWidth;
                            serverScreenWidth=serverScreenHeight;
                            serverScreenHeight=tempVar;
                        }
                        Log.d("Handler","framebufferWidth : "+serverScreenWidth + "== framebufferHeight : " + serverScreenHeight);
                        if (rfb!=null){
                            rfb.setServerScreenSie(serverScreenWidth,serverScreenHeight);
                        }
                        if (orientation.contains("LG")){
                            isLGPhone = true;
                            if (rfb!=null){

                                //服务器为LG手机
                                rfb.setIsLGPhone(isLGPhone);
                            }

                        }
                        if (vncServerOrientationChangedListener != null)
                            if(orientation.contains("rotation:0")){//SCREEN_ORIENTATION_PORTRAIT
                                vncServerOrientationChangedListener.onOrientationChanged("SCREEN_ORIENTATION_PORTRAIT");
                            }else if(orientation.contains("rotation:1")){//SCREEN_ORIENTATION_LANDSCAPE
                                vncServerOrientationChangedListener.onOrientationChanged("SCREEN_ORIENTATION_LANDSCAPE");
                            }else if(orientation.contains("rotation:3")){//SCREEN_ORIENTATION_REVERSE_LANDSCAPE
                                vncServerOrientationChangedListener.onOrientationChanged("SCREEN_ORIENTATION_REVERSE_LANDSCAPE");
                            }
...
//		});
        ...
        Thread t = new Thread() {
            public void run() {
                try {
                    ...
                    handler.post(new Runnable() {
                        public void run() {
                            Log.i(TAG, "Downloading first frame.Please wait...");
    ...
    OnVNCServerOrientationChangedListener vncServerOrientationChangedListener;

    public void registOnServerOrientationChangedListener(OnVNCServerOrientationChangedListener l) {
        this.vncServerOrientationChangedListener = l;
    }

    public void unregistOnServerOrientationChangedListener() {
        this.vncServerOrientationChangedListener = null;
    }

    /**
     * 连接vncserver，并且校验客户端信息
     *
     * @param us
     * @param pw
     * @throws Exception
    ...
    void connectAndAuthenticate(String us, String pw) throws Exception {
        ...
        ///////////////////////////
        //重新设置屏幕信息
        /*rfb.framebufferWidth = 1440;
        rfb.framebufferHeight = 2560;*/


        ////////////////////////////监听服务端屏幕方向变化
        rfb.setOrientationHandler(handler);
    ...
    void doProtocolInitialisation(int dx, int dy) throws IOException {
        Log.i(TAG, "doProtocolInitialisation");
        rfb.writeClientInit();
        rfb.readServerInit();
    ...
    public void processNormalProtocol(final Context context, ProgressDialog pd, final Runnable setModes) throws Exception {
        try {
            Log.i(TAG, "processNormalProtocol");
    ...
    MotionEvent changeTouchCoordinatesToFullFrame(MotionEvent e) {
        ...
        int a = getResources().getDisplayMetrics().heightPixels;
        int b = getMeasuredHeight();
        int top = a - b;
        // Adjust coordinates for Android notification bar.
        e.offsetLocation(0, -1f * top);
        Log.i("davy", "absoluteXPosition=" + absoluteXPosition + ", absoluteYPosition=" + absoluteYPosition);
        Log.i("davy", "real absoluteXPosition=" + (absoluteXPosition + e.getX()) + ", real absoluteYPosition=" + (absoluteYPosition + e.getY()));
        Log.i("davy", "x=" + (e.getX()) + ", y=" + (e.getY()));
        e.setLocation(absoluteXPosition + e.getX() / scale, absoluteYPosition + e.getY() / scale);
//        e.setLocation(absoluteXPosition + e.getX(), absoluteYPosition + e.getY());
    ...
    boolean processPointerEvent(int x, int y, int action, int modifiers, boolean mouseIsDown, boolean useRightButton) {
        ...
        //获取坐标
        Log.d("VncCanvas","x = " + x + "-- y = " + y  );
        if(screenWidth>screenHeight){
            currentX=screenHeight;
            currentY=screenWidth;
        }else {
            currentX=screenWidth;
            currentY=screenHeight;
        }
         if(VncCanvasActivity.currentOrientation.equals("SCREEN_ORIENTATION_PORTRAIT")){

         }else if(VncCanvasActivity.currentOrientation.equals("SCREEN_ORIENTATION_LANDSCAPE")){
             tempY=y;
             y=x;
             x=currentX-tempY;
         }else if(VncCanvasActivity.currentOrientation.equals("SCREEN_ORIENTATION_REVERSE_LANDSCAPE")){
             tempX=x;
             x=y;
             y=currentY-tempX;
         }
        ...
        if (rfb != null && rfb.inNormalProtocol) {
            ...
            Log.d("VncCanvas", "framebufferWidth : "+getFramebufferWidth() + "== framebufferHeight : " + getFramebufferHeight());

            Log.d("VncCanvas","width = "+screenWidth+"---height="+screenHeight);

            //LG获取屏幕信息有问题

            //如果服务端是LG手机 , mouseY = (int)(y*(getFramebufferHeight()*1.0f/screenHeight)+0.5f)+160;
            //不是 , mouseY = (int)(y*(getFramebufferHeight()*1.0f/screenHeight)+0.5f);
            ...
            int x1=getFramebufferWidth();
            int y1=getFramebufferHeight();
            mouseX = (int)(x*(getFramebufferWidth()*1.0f/screenWidth)+0.5f);
            mouseY = (int)(y*(getFramebufferHeight()*1.0f/screenHeight)+0.5f);
//            mouseX = (int)(x*(2560*1.0f/screenWidth)+0.5f);
//            mouseY = (int)(y*(1440*1.0f/screenHeight)+0.5f);

            if (isLGPhone){
                mouseY = (int)(y*(getFramebufferHeight()*1.0f/screenHeight)+0.5f)+160;
                Log.d("VncCanvas","LG手机");
            }

            /*mouseX = (int)(x*(1440*1.0f/1080)+0.5f);
            mouseY = (int)(y*(2720*1.0f/(1920))+0.5f);*/
    ...
    private void initScreenINfo() {
        ...
        Log.d("VncCanvas","screenWidth = "+ screenWidth +"---screenHeight="+ screenHeight);
    }
    ...
    public void getRealScreenSize() {
        ...
        Log.d("SettingActivity","realWidth = "+screenWidth+"---realHeight="+screenHeight);
    }
}
